...
class VersionNumber:
    def __init__(self, filename: str, version_number: str):
...
class Bumper:
    def __init__(self):
        if self._find_bumpy_toml():
            ...
            raise FileNotFoundError('\033[31mbumpy.toml not found in '
            ...
            if file.exists() and file.is_file():
                ...
                if regex_match:
                    version_numbers.append(
                        VersionNumber(filename=file_path,
    ...
    def input_new_version_number(self) -> (list[VersionNumber] | None):
        ...
        while True:
            ...
            if self._match_regex(f"'{new_version_number}'"):
                return [VersionNumber(filename=current.filename,
                                      version_number=current.number[0] + new_version_number + current.number[-1])
    ...
    def bump_version_numbers(self, major: int = 0, minor: int = 0, patch: int = 0) -> None:
        ...
        for current in self.current_versions:
            new_number = current.number[0] + '.'.join([str(current.major + major),
                                                       str(current.minor + minor),
                                                       str(current.patch + patch)]) + current.number[-1]
            if self._match_regex(f"{new_number}"):
                new_versions.append(VersionNumber(filename=current.filename,
                                                  version_number=new_number))
        self.write_version_numbers(new_versions)

    def _print_list_heading(self, heading: str, length_offset: int, version_numbers: list[VersionNumber]):
    ...
    def print_current_version_numbers(self):
        self._print_list_heading(heading='Current Version Numbers',
                                 length_offset=15,
